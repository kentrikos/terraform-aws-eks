...
resource "null_resource" "install_metrics_server" {
  ...
  provisioner "local-exec" {
    command = "helm install metrics-server stable/metrics-server --set args[0]=--kubelet-insecure-tls,args[1]=--kubelet-preferred-address-types=InternalIP  --kubeconfig=${var.outputs_directory}kubeconfig_${var.cluster_prefix}"
  }
